...
-- ---------------------------------------------------------------------------
ALTER TABLE recipes
ADD COLUMN IF NOT EXISTS avg_rating NUMERIC(3, 2) NOT NULL DEFAULT 0 CHECK (
    avg_rating >= 0
    AND avg_rating <= 5
),
ADD COLUMN IF NOT EXISTS rating_count INT NOT NULL DEFAULT 0 CHECK (rating_count >= 0);

CREATE INDEX IF NOT EXISTS idx_recipes_public_rating ON recipes (avg_rating DESC, rating_count DESC)
WHERE
    deleted_at IS NULL
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_ingredients_name_lower ON ingredients (lower(name))
WHERE
    deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_recipe_step_pos ON recipe_steps (recipe_id, position)
WHERE
    deleted_at IS NULL;
...
CREATE TABLE
    IF NOT EXISTS tags (
        ...
        kind VARCHAR(32) NOT NULL DEFAULT 'general' CHECK (
            kind IN ('cuisine', 'diet', 'occasion', 'general')
        )
    );
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS user_name VARCHAR(24);
ALTER TABLE users ADD COLUMN IF NOT EXISTS hash_pass VARCHAR(255);

UPDATE users
SET user_name = 'user_' || substr(replace(id::text, '-', ''), 1, 16)
WHERE user_name IS NULL OR user_name = '';

ALTER TABLE users ALTER COLUMN user_name SET NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_users_user_name_lower ON users (lower(user_name))
